  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-body">
      ...
      <motion.div 
        ...
        className="pinterest-card w-full max-w-md p-10 sm:p-14 mb-10"
      >
        <div className="flex flex-col items-center mb-10">
          <div className="w-16 h-16 bg-primary rounded-[2rem] flex items-center justify-center shadow-lg transform -rotate-6 hover:rotate-6 transition-transform duration-300">
            <UserPlus className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-4xl font-bold tracking-tight mt-8">Pin Board</h1>
          <p className="text-text-muted mt-3 text-lg">Join the visual workspace</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="flex flex-col gap-3">
            <label className="text-sm font-bold text-text-muted ml-2 uppercase tracking-widest">Pin Name</label>
            <div className="relative group">
              <User className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-text-muted group-focus-within:text-primary transition-colors duration-300" />
              <input 
                type="text" 
                className="input-minimal pl-14 py-4" 
                placeholder="Jan Pinster"
                value={name}
        ...
        <div className="mt-12 pt-10 border-t border-border-color">
          <p className="text-center text-text-muted font-medium">
            Already pinned?{' '}
            <Link to="/login" className="text-primary hover:text-primary-hover font-black text-lg transition-colors ml-1 decoration-primary underline-offset-4 hover:underline">
              Log in
            </Link>
          </p>
        </div>
